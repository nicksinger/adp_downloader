...
class DB():
  def __init__(self):
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS companies (id INTEGER PRIMARY KEY AUTOINCREMENT, company text NOT NULL, UNIQUE(company))''')
    c.execute('''CREATE TABLE IF NOT EXISTS persons (id INTEGER PRIMARY KEY AUTOINCREMENT, staff_number text NOT NULL, UNIQUE(staff_number))''')
    c.execute('''CREATE TABLE IF NOT EXISTS document_types (id INTEGER PRIMARY KEY AUTOINCREMENT, type text NOT NULL, UNIQUE(type))''')
    c.execute('''CREATE TABLE IF NOT EXISTS documents (company_id INTEGER, person_id INTEGER, document_type INTEGER, document_date TEXT, download_date TEXT, FOREIGN KEY(company_id) REFERENCES companies(id), FOREIGN KEY(person_id) REFERENCES persons(id), FOREIGN KEY(document_type) REFERENCES document_types(id), UNIQUE(company_id, person_id, document_type, document_date))''')
    self.connection.commit()

  def persist(self, adpdocument):
    c = self.connection.cursor()
    c.execute("""INSERT OR IGNORE INTO companies(company) VALUES(?)""", (adpdocument.company_id,))
    c.execute("""INSERT OR IGNORE INTO persons(staff_number) VALUES(?)""", (adpdocument.employee_nr,))
    c.execute("""INSERT OR IGNORE INTO document_types(type) VALUES(?)""", (adpdocument.type,))
    company_id, employee_id, type_id = self.query_indices(adpdocument.company_id, adpdocument.employee_nr, adpdocument.type)
    c.execute("""INSERT OR IGNORE INTO documents (company_id, person_id, document_type, document_date, download_date) VALUES (?, ?, ?, DATE(?), DATETIME(?));""", (company_id, employee_id, type_id, adpdocument.date.strftime('%Y-%m-%d'), datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')));
    self.connection.commit()
  ...
  def query_indices(self, company_id, employee_nr, document_type):
    ...
    try:
      ...
      c.execute("""SELECT id FROM document_types WHERE type=?""", (document_type,));
  ...
  def document_present(self, adpdocument):
    ...
    try:
      company_id, employee_id, type_id = self.query_indices(adpdocument.company_id, adpdocument.employee_nr, adpdocument.type)
      result = c.execute("""SELECT download_date FROM documents WHERE person_id = ? AND document_type = ? AND company_id = ? AND document_date = ?""", (employee_id, type_id, company_id, adpdocument.date.strftime('%Y-%m-%d')))
